...
use crate::{
    ...
    position::{num_line_symmetries, num_square_symmetries, starting_capstones, Komi},
};

use super::policy_eval::policy_offset;

pub const NUM_VALUE_FEATURES_4S: usize = 336;
pub const NUM_POLICY_FEATURES_4S: usize = 192;

pub const NUM_VALUE_FEATURES_5S: usize = 536;
pub const NUM_POLICY_FEATURES_5S: usize = 224;

pub const NUM_VALUE_FEATURES_6S: usize = 560;
pub const NUM_POLICY_FEATURES_6S: usize = 232;
...
impl<const S: usize> ValueIndexes<S> {
    ...
    pub const fn new() -> Self {
        let i = 0;
        // 1 if the board size has capstone at all, 0 otherwise
        let capstones_factor = (starting_capstones(S) != 0) as usize;
        ...
        let (cap_psqt_opening, i) =
            IndexPair::next(i, num_square_symmetries::<S>() * capstones_factor);
        let (cap_psqt_middlegame, i) =
            IndexPair::next(i, num_square_symmetries::<S>() * capstones_factor);
        let (cap_psqt_endgame, i) =
            IndexPair::next(i, num_square_symmetries::<S>() * capstones_factor);
        ...
        let (critical_square_cap_attack, i) = IndexPair::next(i, 6 * capstones_factor);
        ...
        let (sidelined_cap, i) = IndexPair::next(i, 3 * capstones_factor);
        let (fully_isolated_cap, i) = IndexPair::next(i, 3 * capstones_factor);
        let (semi_isolated_cap, i) = IndexPair::next(i, 3 * capstones_factor);
...
pub struct PolicyIndexes<const S: usize> {
    pub first_ply_psqt: IndexPair,
    pub second_ply_psqt: IndexPair,
...
impl<const S: usize> PolicyIndexes<S> {
    ...
    pub const fn new() -> Self {
        let i = 0;
        let (first_ply_psqt, i) = IndexPair::next(i, num_square_symmetries::<S>());
        let (second_ply_psqt, i) = IndexPair::next(i, num_square_symmetries::<S>());
