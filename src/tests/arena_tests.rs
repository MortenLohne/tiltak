...
fn supports_type_test() {
    ...
    assert_eq!(unsafe { *arena.get(&index) }, 42);
}
...
fn slice_test() {
    ...
    assert_eq!(unsafe { arena.get_slice(&slice_index) }, &[1, 2, 3]);
    assert_eq!(unsafe { *arena.get(&index) }, [4, 5]);
    assert_eq!(unsafe { *arena.get(&index2) }, [6, 7, 8]);
}
...
fn slot_57_size() {
    ...
    assert_eq!(unsafe { arena.get(&index) }, &[3; 57]);
    assert_eq!(
        unsafe { arena.get_slice(&slice_index) },
        &[[4; 57], [5; 57]]
    );
}
